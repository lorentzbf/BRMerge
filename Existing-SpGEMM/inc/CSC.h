#ifndef YUSUKE_CSC_H_
#define YUSUKE_CSC_H_
...
public:
    ...
    CSC (IT *col_pointer, IT* row_index, NT* csc_values, IT M, IT N, IT nnz, int my);
...
CSC<IT, NT>::CSC (IT *col_pointer, IT* row_index, NT* csc_values, IT M, IT N, IT nnz_, int my){
    rows = M + my -my;
    cols = N;
    nnz = nnz_;
    colptr = my_malloc<IT>(cols + 1);
    rowids = my_malloc<IT>(nnz);
    values = my_malloc<NT>(nnz);
    memcpy(colptr, col_pointer, (cols + 1)*sizeof(IT));
    memcpy(rowids, row_index, nnz*sizeof(IT));
    memcpy(values, csc_values, nnz*sizeof(NT));
}
