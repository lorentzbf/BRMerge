#ifndef YUSUKE_HEAP_SPGEMM_H
#define YUSUKE_HEAP_SPGEMM_H
...
#pragma omp parallel
    ...
    long long int flops = get_flop(A, B, maxnnzc);
...
template <typename IT, typename NT, typename MultiplyOperation, typename AddOperation>
void prof_HeapSpGEMM(const CSC<IT,NT> & A, const CSC<IT,NT> & B, CSC<IT,NT> & C, MultiplyOperation multop, AddOperation addop, int prof = 0)
{
    double t0, t1;
    t0 = t1 = get_wc_time();
    if(prof)
...
#pragma omp parallel
    ...
    if(prof)
    printf("parallel preallocate time %le\n", get_wc_time() - t0);
...
#pragma omp parallel
    ...
    if(prof)
    printf("parallel compute time %le\n", get_wc_time() - t0);
...
#pragma omp parallel
    ...
    if(prof)
    printf("parallel deallocate time %le\n", get_wc_time() - t0);
    ...
    my_free<IT>(colStart);
    if(prof)
    printf("post deallocate time %le\n", get_wc_time() - t0);
