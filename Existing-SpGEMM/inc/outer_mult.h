    ...
    int nblockers, int nblockchars)
{
    ...
    //IT* row_blocker_begin_ptr = my_malloc<IT>(num_blockers);
    //IT* row_blocker_end_ptr   = my_malloc<IT>(num_blockers);
    //IT* flop_groupby_row_blockers = my_malloc<IT>(num_blockers);
    ...
    //IT* flop_groupby_col_blockers = my_malloc<IT>(num_blockers * num_blockers);
    ...
    TripleNode* global_blockers = my_malloc<TripleNode>(total_flop);
    ...
    TripleNode* local_blockers = my_malloc<TripleNode>(block_width * num_blockers * nthreads);
    IT* size_of_local_blockers = new IT[num_blockers * nthreads]();
    //IT* size_of_local_blockers = my_malloc<IT>(num_blockers * nthreads);
...
#pragma omp for
    ...
    for (uint16_t row_blocker_index = 0; row_blocker_index < num_blockers; ++row_blocker_index)
        ...
        flop_space[row_blocker_index] = my_malloc<TripleNode>(flop_groupby_row_blockers[row_blocker_index]);
    ...
    IT* nnz_per_col_blocker = my_malloc<IT>(num_blockers * num_blockers);
    IT* col_blocker_begin_ptr = my_malloc<IT>(num_blockers * num_blockers);
    IT* col_blocker_end_ptr = my_malloc<IT>(num_blockers * num_blockers);
    ...
    TripleNode* sorting_buffer = my_malloc<TripleNode>(max_flops_in_col_blockers * nthreads + 1);
...
#pragma omp for
    ...
    //IT *cumulative_colid_indices = my_malloc<IT>(num_blockers * num_blockers + 1);
    //IT *cumulative_col_blocker_indices = my_malloc<IT>(num_blockers * num_blockers + 1);
    ...
    C.colids = my_malloc<IT>(total_nnz);
    C.values = my_malloc<NT>(total_nnz);
    ...
    delete [] size_of_local_blockers;
    delete [] row_blocker_begin_ptr;
    delete [] row_blocker_end_ptr;
    delete [] flop_groupby_row_blockers;
    delete [] flop_groupby_col_blockers;
    delete [] nnz_by_row;
    delete [] nnz_per_row_blocker;
    //delete [] nnz_per_col_blocker;
    //delete [] col_blocker_begin_ptr;
    //delete [] col_blocker_end_ptr;
    delete [] cumulative_colid_indices;
    delete [] cumulative_col_blocker_indices;
    for (uint16_t row_blocker_index = 0; row_blocker_index < num_blockers; ++row_blocker_index)
