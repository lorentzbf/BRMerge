...
CSR::CSR(const CSR &A, const std::vector<int> &rows){
    for(int i : rows){
        if(i >= A.M){
            printf("error row %d, max row %d\n", i, A.M);
            assert(0);
        }
    }
    M = rows.size();
    ...
    r = 0;
    for(mint i : rows){
        memcpy(col + rpt[r], A.col + A.rpt[i], row_size[r]*sizeof(mint));
        memcpy(val + rpt[r], A.val + A.rpt[i], row_size[r]*sizeof(mdouble));
        r++;
    }
    delete [] row_size;
}
...
//template CSR<double>::CSR(const std::string &);
//template CSR<double>::CSR(const CSR<double>&);
//template void CSR<double>::hrelease();
//template void CSR<double>::drelease();
//template void CSR<double>::release();
//template void CSR<double>::D2H();
//template void CSR<double>::H2D();
//template bool CSR<double>::operator==(const CSR<double>&);
